...
class _LogControls extends StatelessWidget {
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                ...
                Text('Lines:', style: AppTextStyles.caption),
                const SizedBox(width: 8),
                _buildTailDropdown(),
              ],
            ),
  ...
  Widget _buildTimeChip(String label, String? sinceValue) {
    ...
    return ChoiceChip(
      ...
      selected: isSelected,
      onSelected: (selected) {
        if (selected) provider.setLogSince(sinceValue);
      },
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<int>(
          ...
          onChanged: (int? newValue) {
            if (newValue != null) provider.setLogTail(newValue);
          },
          items: const [
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: disabled ? AppColors.glassOverlay : color.withValues(alpha: 0.12),
          borderRadius: BorderRadius.circular(19),
          border: Border.all(
            color: disabled ? AppColors.glassBorder : color.withValues(alpha: 0.4),
          ),
        ),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(icon, size: 16, color: disabled ? AppColors.textSubtle : color),
            const SizedBox(width: 6),
            Text(
              label.toUpperCase(),
              style: TextStyle(
...
class LogControls extends StatelessWidget {
  ...
  const LogControls({super.key, required this.isFollowingLogs, required this.hasLogs, this.onToggleFollow, this.onRefresh});
...
class LogDisplay extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => _LogDisplay(logs: logs, searchQuery: '');
}