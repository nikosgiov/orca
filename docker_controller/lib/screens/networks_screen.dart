    ...
    return Container(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.push(
          context,
          AppTransitions.focalZoom(const CreateNetworkScreen()),
        ),
      ...
      body: RefreshIndicator(
        ...
        child: provider.isLoadingNetworks
            ...
            : provider.networks.isEmpty
                ? Center(
                    child: Text('No networks found', style: AppTextStyles.caption),
                  )
            : ListView.builder(
...
class _NetworkCard extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.account_tree_outlined, color: Color(0xFF60A5FA), size: 20),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('$driver · ${AppStrings.labelScope}: $scope', style: AppTextStyles.caption),
                  ],
                ),
              ),
            ],
          ...
          InfoRow(label: AppStrings.labelSubnet, value: subnet, icon: Icons.network_check_outlined),
          InfoRow(label: AppStrings.labelContainers, value: containers, icon: Icons.dns_outlined),
          const SizedBox(height: 12),
          Row(
            children: [
  ...
  void _inspect(BuildContext ctx, Map<String, dynamic> network, VolumesNetworksProvider provider) async {
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _InspectRow(label: 'Name', value: details['Name']?.toString() ?? ''),
                _InspectRow(label: 'Driver', value: details['Driver']?.toString() ?? ''),
                _InspectRow(label: 'Scope', value: details['Scope']?.toString() ?? ''),
                _InspectRow(label: 'Created', value: details['Created']?.toString() ?? ''),
                _InspectRow(label: 'Subnet', value: _extractSubnet(details)),
                _InspectRow(label: 'Containers', value: _extractContainers(details)),
              ],
            ),
          ),
        ),
  ...
  void _remove(BuildContext ctx, String name, VolumesNetworksProvider provider) {
    showDialog(
      context: ctx,
  ...
  static String _extractSubnet(Map<String, dynamic> network) {
    ...
    if (ipam is Map && ipam['Config'] is List && (ipam['Config'] as List).isNotEmpty) {
      ...
      if (config is Map && config['Subnet'] != null) return config['Subnet'].toString();
    }
    return 'N/A';
  }
