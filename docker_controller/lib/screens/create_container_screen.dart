    ...
    return Consumer<AppProvider>(
      builder: (context, appProvider, _) {
        return ChangeNotifierProvider(
          create: (_) => CreateContainerProvider(appProvider, preSelectedImage: preSelectedImage),
                    ...
                    SafeArea(
                      ...
                      child: StepNavigationButtons(
                        ...
                        onNextOrCreate: provider.currentStep == provider.stepTitles.length - 1
                            ? () async {
                                final containerId = await provider.createContainer(context);
                                if (containerId != null) {
                                  final status = provider.startAfterCreate ? 'created and started' : 'created';
                                  if (!context.mounted) return;
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Container $status successfully! ID: \${containerId.substring(0, 12)}'),
