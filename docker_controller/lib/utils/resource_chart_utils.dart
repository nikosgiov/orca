...
class ResourceChartUtils {
  static List<FlSpot> getChartData(List<ResourceDataPoint> history, String dataType, String selectedTimeRange) {
    ...
    return filteredData.asMap().entries.map((entry) {
      ...
      switch (dataType) {
        ...
        case 'disk':
          if (index == 0) return FlSpot(index.toDouble(), 0);
          final prevDisk = filteredData[index - 1];
        ...
        case 'network':
          if (index == 0) return FlSpot(index.toDouble(), 0);
          final prevNet = filteredData[index - 1];
  ...
  static String getCurrentValue(List<ResourceDataPoint> history, String dataType) {
    ...
    switch (dataType) {
      ...
      case 'disk':
        if (history.length < 2) return '0.0 MB/s';
      ...
      case 'network':
        if (history.length < 2) return '0.0 KB/s';
        final prev = history[history.length - 2];
        final sentDiff = (latest.netSent ?? 0) - (prev.netSent ?? 0);
        final recvDiff = (latest.netRecv ?? 0) - (prev.netRecv ?? 0);
        final speed = (sentDiff >= 0 ? sentDiff : 0.0) + (recvDiff >= 0 ? recvDiff : 0.0);
        return _formatSpeed(speed);
      case 'gpu':
  ...
  static List<ResourceDataPoint> fromMetricsArrays(Map<String, dynamic> metrics, [String selectedTimeRange = '30m']) {
    final int length = (metrics['cpu'] as List).length;
    ...
    for (int i = 0; i < length; i++) {
      points.add(ResourceDataPoint(
        timestamp: DateTime.now().subtract(step * (length - 1 - i)),
        cpuUsage: (metrics['cpu']?[i] ?? 0.0).toDouble(),
        memoryUsage: (metrics['mem']?[i] ?? 0.0).toDouble(),
        diskUsage: (metrics['disk']?[i] as num?)?.toDouble(),
        diskIO: (((metrics['disk_read']?[i] as num?)?.toDouble() ?? 0.0) + ((metrics['disk_write']?[i] as num?)?.toDouble() ?? 0.0)),
        netSent: (metrics['net_sent']?[i] ?? 0.0).toDouble(),
        netRecv: (metrics['net_recv']?[i] ?? 0.0).toDouble(),
        gpuUsage: (metrics['gpu_load']?[i] as num?)?.toDouble(),
      ));
    }
    return points;
  }
